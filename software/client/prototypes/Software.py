...
from PyQt5.QtWidgets import QWidget, QColorDialog, QSlider, QLabel, QPushButton, QToolButton, QLineEdit, QInputDialog, QMainWindow, QComboBox
...
class MainWindow(QtWidgets.QMainWindow):
        ...
        if not editMode:
            ...
            for r in range (6):
                for c in range (10):
                    ...
                    self.ui.gridLayout_4.addWidget(button, r, c, 1, 1)
    ...
    def addRow(self):
        ...
        cue = Cue(self, num, len(cueList) + 1, "", anim, offGrid.copy())
        cueList.append(cue)
                        ...
                        if cue[0] == "S":
                            ...
                            for row in rowList:
                                ...
                                for col in colList:
                                    ...
                                    rgb[2] = int(rgb[2])
                                    r.append(rgb)
                            cueList.append(Cue(self, numShown, len(cueList) + 1, label, False, gridList))
    ...
    def setCubeColor(self): 
        ...
        if gridExists:
            ...
            pass #update the grid on the screen as well???
...
class Cue():
    def __init__(self, mainUI, numShown, index, label, animtrue, grid, anim = {}, animlen = 0, transType = "None", fadelen = 0, trans = {}, translen = 0): #gridlayout, gridlayoutwidget, self, cue number, cue label (s - or a -), animation or static, grid if it's static, animation, animation length, fade true or false, fade length (won't check unless fade is true), fancy transition, fancy transition length
    #when creating new cue, add one to existing last number, label = s - or a - and anim value from popup, if from an existing cue, put in stuff about animations and transitions, otherwise, all blank
        ...
        jumpButton = QPushButton(mainUI.ui.gridLayoutWidget)
        mainUI.ui.gridLayout.addWidget(jumpButton, self.index + 1, 0, 1, 1)
        jumpButton.clicked.connect(self.jumpTo)
        
        numberLine = QLineEdit(str(numShown), mainUI.ui.gridLayoutWidget)
        mainUI.ui.gridLayout.addWidget(numberLine, self.index + 1, 1, 1, 1)
        numberLine.textEdited.connect(lambda: self.updateNumber(numberLine.text()))
        ...
        if label[0:2] != "A-" and label [0:2] != "S-":
            if self.animtrue:
                ...
                label = "S-" + label
        labelLine = QLineEdit(label, mainUI.ui.gridLayoutWidget)
        mainUI.ui.gridLayout.addWidget(labelLine, self.index + 1, 2, 1, 1)
        labelLine.textEdited.connect(lambda: self.updateLabel(labelLine))
        
        gridEditButton = QPushButton(mainUI.ui.gridLayoutWidget)
        mainUI.ui.gridLayout.addWidget(gridEditButton, self.index + 1, 3, 1, 1)
        gridEditButton.clicked.connect(self.changeGrid)
        
        transDrop = QComboBox(mainUI.ui.gridLayoutWidget)
        transDrop.addItems(["None", "Fade", "Snake", "Create New"]) #add snake to possibilities in file!!
        mainUI.ui.gridLayout.addWidget(transDrop, self.index + 1, 4, 1, 1)
        
        transEditButton = QPushButton(mainUI.ui.gridLayoutWidget)
        mainUI.ui.gridLayout.addWidget(transEditButton, self.index + 1, 5, 1, 1)
        transEditButton.clicked.connect(self.changeTrans)
