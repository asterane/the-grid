            ...
            for s in range(steps): #add one to s when using it because it'll start at 0
                for r in range(6):
                    for c in range(10):
                        ...
                        stepG = diffG / steps
                        stepB = diffB / steps
                        newR = firstGrid[r][c][0] + stepR * (s + 1)
                        newG = firstGrid[r][c][1] + stepG * (s + 1)
                        newB = firstGrid[r][c][2] + stepB * (s + 1)
...
class Cue():
    ...
    #when creating new cue, add one to existing last number, label = s - or a - and anim value from popup, if from an existing cue, put in stuff about animations and transitions, otherwise, all blank
        ...
        #numberLine.returnPressed.connect(lambda: self.updateNumber(numberLine.text()))
        ...
        mainUI.ui.gridLayout.addWidget(self.transDrop, self.index + 1, 4, 1, 1)
